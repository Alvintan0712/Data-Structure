...
unsigned int hash(char *s)
{
    unsigned int seed = 131, val = 0, i = 0;
...
void Article_Insert(node *head[], char *s)
{
    unsigned int n;
    node *p, *q;
...
void Print()
{
    ...
    fprintf(out,"%.5lf\n\n", sim);
    for(i = 0; i < ttop1; i++) fprintf(out, "%s %d\n", Array1[i].s, Array1[i].cnt); fprintf(out,"\n");
    for(i = 0; i < ttop2; i++) fprintf(out, "%s %d\n", Array2[i].s, Array2[i].cnt);
